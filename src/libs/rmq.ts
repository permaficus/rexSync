...
class RabbitInstance extends EventEmitter {
  ...
  private calculateRetryInterval(): number {
    return Math.min(this.MIN_RETRY_INTERVAL * Math.pow(2, this.attempt), this.MAX_RETRY_INTERVAL)
  }
  ...
  public async initiateExchange({ ...args }: BrokerExchangeInterface): Promise<{ exchange: string; url: string }> {
    try {
      await args.channel.assertExchange(args.name || this.defaultExchange, args.type, {
...
async function sendMessage(
  ...
  rbmq.on('ENOTFOUND', () => {
    console.error(
      `[RBMQ] Error: cannot reach ${url.split('@')[1].split(`/`)[0]}.\n[RBMQ] Please check your internet connection}`
    )
  })
