...
import { EventPayload, RexSyncInitArguments, AuthSchemes, BrokerExchangeInterface, QueueTypeInterface } from '../types'
...
class RexSync {
  private args: RexSyncInitArguments
  private client: RedisClientType
  private redisChannel: string

  constructor(args: RexSyncInitArguments) {
    this.args = args
    this.validateUrls()
    this.init()
  }
  ...
  private validateRabbitMQConfig(
    ...
    if (!exchange || !queue || !routing) {
      throw new RexSyncError("RabbitMQ transport requires 'exchange', 'queue', and 'routing' parameters.")
    }
  }
